...
if ($env:USERNAME -ne "ADSHClass" -or $env:USERDOMAIN -ne "ADSHClass") {
    Write-Host "`n`nWARN: Wrong user. Expected: ADSHClass\Administrator, Found: $env:USERDOMAIN\$env:USERNAME. Exiting...`n`n"
    pause
    Stop-Process -Id $PID
}
